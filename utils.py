...
def get_data(country, indices=[], demand=[0], temp=[0], spv=[0], rollout_values=[0], n_futures=24*31):
    ...
    full_time_index = pd.date_range(
        start=historical_data.index.min(), end=pd.Timestamp("2024-08-31 23:00:00"), freq="h"
    )
    ...
    for i, time in enumerate(full_time_index):
        # Check if the date of the current time matches any holiday or weekend (ignore time part)
    ...
    dataframes[0] = pd.concat([dataframes[0], pd.DataFrame({"DATETIME": pd.date_range(start=pd.Timestamp("2024-08-01 00:00:00"), end=pd.Timestamp("2024-08-31 23:00:00"), freq="h")})], ignore_index=True)
    ...
    for i in tqdm(indices):
        ...
        for i in range(1, n_futures+1):
            ...
            new_df['hour_future' + str(i)] = new_df['hour'].shift(-i)
            new_df['day_future' + str(i)] = new_df['day'].shift(-i)
            new_df['month_future' + str(i)] = new_df['month'].shift(-i)
        ...
        for i in range(n_futures+1):
            if i == 0:
                ...
                y[f"future{i}"] = new_df[target_col].shift(-i)

        times = new_df["DATETIME"]
        new_df = new_df.drop(columns=["DATETIME"]).reset_index(drop=True)
        

        X = new_df.drop(columns=[target_col]).reset_index(drop=True)
